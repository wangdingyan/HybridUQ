...
def split_data(data        :  MoleculeDataset,
               ...
               seed        :  int = 0,
               num_folds   :  int = 1,  # only needed for "cv" split
               args        :  TrainArgs = None,
...
def get_features_split_index(features,
                             cluster_num=5,
                             cluster_type='random'):
    ...
    if cluster_type=='random':
        ...
        part_size = n_features // cluster_num
        for c in range(cluster_num):
            if c == 0:
                cluster_size = n_features - part_size * cluster_num
            else:
                cluster_size = part_size
    ...
    elif cluster_type=='acluster':
        # Y = pdist(features, 'correlation')
        # print(Y)
        # l = linkage(Y, 'complete')
        # clusters = fcluster(l, 0.75 * Y.max(), 'distance')
        #
        X = []

        clusters = AgglomerativeClustering(n_clusters=cluster_num).fit(features).labels_
        ...
        raise ValueError(f'cluster_type "{cluster_type}" not supported.')